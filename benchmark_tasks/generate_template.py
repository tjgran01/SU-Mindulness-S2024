...
def main(id_prefixes=[10, 20]):
    ...
    sessions = ["1", "2"]
    templates = ["1", "2", "3", "4"]
    practice = ["0" for elm in _subj_ids]
    ses = [sessions[i % len(sessions)] for i, elm in enumerate(_subj_ids)]
    temp = [templates[i % len(templates)] for i, elm in enumerate(_subj_ids)]
    ...
    with open ("out.csv", "w", newline='') as out_fp:
        ...
        for i, elm in enumerate(_subj_ids):
